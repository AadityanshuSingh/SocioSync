...
export function updateChat(query) {
    return async (dispatch) => {
        // dispatch(setLoading(true))
        try {
        const response = await apiConnector("PUT", SEND_CHAT_REQUEST_API, {query});
...
export function getHistory(query) {
    return async (dispatch) => {
        ...
        const response = await apiConnector("GET", GET_CHAT_HISTORY_API, null, null, {query});
        ...
        if (!response.data.success) {
            ...
            console.log("GET_CHAT_HISTORY_API API RESPONSE.........", response.data);
        }
